...
def sacar_comments(text):
    text = subprocess.check_output(
        "gcc -fpreprocessed -dD -E -".split(), input=text.encode()
    )
    return text.decode()
...
def main(c_source, *args):
    ...
    text = text.replace(
        "uint64_t", "int"
    )  # Estos tipos son placeholder, no importa cual usemos. Se reemplaza para que compile sin librerías.
    ...
    out = []
    out.append(
        f"""// generado por la cosa rara de la abi
#include "stddef.h"
#include "stdio.h"
#include "stdint.h"
